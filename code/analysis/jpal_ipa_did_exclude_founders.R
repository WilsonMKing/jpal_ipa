...
freqs_panel <- create_freqs_panel(
  wos_data %>% 
    dplyr::filter(!grepl("Banerjee, A|Duflo, E|Kremer, M|Mullainathan, S|Glennerster, R|Karlan, D", 
                         wos_data$authors)))  %>%
...
### At Least One RCT
reg4 <- run_ipa_regressions(outcome = "ifelse(frequency_experiments > 0, 1, 0)", specification = "ols-binary-did")
reg5 <- run_jpal_regressions(outcome = "ifelse(frequency_experiments > 0, 1, 0)", specification = "ols-binary-did")
reg6 <- run_jpal_regressions(outcome = "ifelse(frequency_experiments > 0, 1, 0)", specification = "ols-binary-did-both")

### Number of RCTs
reg1 <- run_ipa_regressions(outcome = "frequency_experiments", specification = "ols-binary-did")
reg2 <- run_jpal_regressions(outcome = "frequency_experiments", specification = "ols-binary-did")
reg3 <- run_jpal_regressions(outcome = "frequency_experiments", specification = "ols-binary-did-both")
