...
for(i in 1:1000){
  ...
  ### Run Regression
  reg1 <- run_jpal_regressions(outcome = "frequency_rct", data = freqs_panel_jpal_ipa, specification = "ols-binary-did")
  reg2 <- run_ipa_regressions(outcome = "frequency_rct", data = freqs_panel_jpal_ipa, specification = "ols-binary-did")
  
  ### Save Coefficient
  placebo_coefficients1[i] <- reg1$coefficients[1]
  placebo_coefficients2[i] <- reg2$coefficients[1]
  
}


real_reg1 <- run_jpal_regressions(outcome = "frequency_rct", data = freqs_panel, specification = "ols-binary-did")
real_reg2 <- run_ipa_regressions(outcome = "frequency_rct", data = freqs_panel, specification = "ols-binary-did")
...
ggsave(filename = paste0(exhibits, "figures/randomization_test_ipa.jpeg"), plot = last_plot(),
...
ggsave(filename = paste0(exhibits, "figures/randomization_test_jpal.jpeg"), plot = last_plot(),
       units = "cm", width = 20, height = 10)
