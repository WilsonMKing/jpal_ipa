...
event_study_results <- 
  data.frame(
    outcome = 
      c(rep("Abdul Latif Jameel Poverty Action Lab", 46), 
        rep("Innovations for Poverty Action", 46)),
    coef = 
      c(as.numeric(twfe_ols[["jpal"]]$coefficients[16:37]), 0, 
        as.numeric(twfe_sa[["jpal"]]$coeftable[,1][16:37]), 0,
        as.numeric(twfe_ols[["ipa"]]$coeftable[,1][15:36]), 0,
        as.numeric(twfe_sa[["ipa"]]$coeftable[,1][15:36]), 0),
    se = 
      c(as.numeric(twfe_ols[["jpal"]]$se[16:37]), 0, 
        as.numeric(se(twfe_sa[["jpal"]])[16:37]), 0,
        as.numeric(twfe_ols[["ipa"]]$se[15:36]), 0, 
        as.numeric(se(twfe_sa[["ipa"]])[15:36]), 0),
    estimator = 
      c(rep("OLS", 23), 
        rep("Sun & Abraham", 23),
        rep("OLS", 23), 
